...
if ($LASTEXITCODE -eq 0) {
    Write-Host "OK: Navi tests passed" -ForegroundColor Green
} else {
    Write-Host "FAIL: Navi tests failed" -ForegroundColor Red
    exit 1
}
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "OK: Psykologen tests passed" -ForegroundColor Green
} else {
    Write-Host "FAIL: Psykologen tests failed" -ForegroundColor Red
    exit 1
}
Write-Host ""

Write-Host "3. Verifying Ingenioren module tests..." -ForegroundColor Yellow
python -m pytest tests/test_ingenioren.py -v --tb=short -q
if ($LASTEXITCODE -eq 0) {
    Write-Host "OK: Ingenioren tests passed" -ForegroundColor Green
} else {
    Write-Host "FAIL: Ingenioren tests failed" -ForegroundColor Red
    exit 1
}
Write-Host ""
