...
const MyComponent = () => {
  ...
  return (
    <div>
      ...
      <p>Heading: {latestData?.navigation?.heading}Â°</p>
      <p>Water Depth: {latestData?.environment?.water_depth}m</p>
      <p>Wind: {latestData?.environment?.wind_speed} m/s at {latestData?.environment?.wind_direction}Â°</p>
...
- **IngeniÃ¸ren**: Monitor engine parameters for diagnostics
...
**Ready to use modern marine data? Configure Signal K and sail into the future!** âš“
